...
class FinancingVariableResolver
{
    ...
    private function buildRawVars(FinancingRequest $financing, string $locale): array
    {
        ...
        $vars = [
            ...
            '{montant_totalavecinteret}' => number_format((float)$financing->total_with_interest, 2, ',', ' '),
            '{devise}'          => $financing->currency ?? 'EUR',
            '{duree}'           => (string) ($financing->duration_months ?? ''),
            '{mensualite}'      => number_format((float)$financing->monthly_payment, 2, ',', ' '),
            '{montant_mensualite}' => number_format((float)$financing->monthly_payment, 2, ',', ' '),
            '{taux}'            => number_format((float)$financing->interest_rate, 2, ',', '.'),
