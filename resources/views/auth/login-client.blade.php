...
.ls__btn:hover{border-color:var(--gold);background:#F6EFD8}
...
.ls__menu{
  ...
  border-radius:12px;box-shadow:0 16px 48px rgba(7,26,51,.14);
...
.ls__opt.cur{background:#F6EFD8;color:var(--gold2)}
...
.card{
  ...
  box-shadow:0 4px 24px rgba(7,26,51,.06),0 16px 48px rgba(7,26,51,.08);
}
...
.logo-box{
  ...
  box-shadow:0 0 36px rgba(7,26,51,.22);
}
...
.finput:focus{
  ...
  box-shadow:0 0 0 3.5px rgba(7,26,51,.08);
}
...
.fbtn:hover{
  ...
  box-shadow:0 8px 36px rgba(7,26,51,.35),0 2px 10px rgba(7,26,51,.2);
}
...
.qavatar{
  ...
  box-shadow:0 0 0 4px rgba(7,26,51,.1);
}
...
.pwa-icon{
  ...
  background:linear-gradient(135deg,var(--navy2),var(--navy));
  display:flex;align-items:center;justify-content:center;
  box-shadow:0 4px 14px rgba(7,26,51,.25);
}
...
#pwa-ios{
  display:none;
  margin:0 1.25rem .5rem;
  background:rgba(201,162,39,.06);border:1.5px solid rgba(201,162,39,.2);
...
<div class="page-shell">
  ...
  <div class="topbar">
    ...
    @php
      $cur   = app()->getLocale();
      $langs = ['fr'=>['Français','png'],'en'=>['English','png'],'pl'=>['Polski','svg'],'es'=>['Español','png'],'bg'=>['Български','png'],'hu'=>['Magyar','png'],'it'=>['Italiano','png'],'de'=>['Deutsch','png'],'lt'=>['Lietuvių','png'],'ro'=>['Română','png'],'lv'=>['Latviešu','png'],'nl'=>['Nederlands','png'],'pt'=>['Português','png'],'sk'=>['Slovenčina','png'],'el'=>['Ελληνικά','png']];
    @endphp
