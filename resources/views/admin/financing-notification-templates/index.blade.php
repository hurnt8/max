...
@section('title','Modèles de notification — Financement')
@section('page_title','Modèles de notification — Financement')

@section('content')

<div class="d-flex align-items-start justify-content-between flex-wrap gap-3 page-hdr">
  <div>
    <h4>Modèles de notification — Financement</h4>
    <p>Un modèle par langue et par étape — utilisé automatiquement selon la langue du client au moment de l'envoi. Indépendant des modèles « Prêt ».</p>
  </div>
  <a href="{{ route('admin.financing-notification-templates.create') }}" class="btn-navy">
    <i class="fas fa-plus"></i> Nouveau modèle
  </a>
</div>

<div style="display:flex;gap:.4rem;margin-bottom:1.25rem">
  @foreach(\App\Models\FinancingNotificationTemplate::TYPES as $code => $label)
  <a href="{{ route('admin.financing-notification-templates.index', ['type' => $code]) }}"
...
<div class="card-pro" style="text-align:center;padding:4rem 2rem">
  ...
  <p style="color:var(--c-muted);font-size:.8375rem;margin-bottom:1.25rem">
    Créez au moins un modèle en français (utilisé en repli pour les langues non configurées) avant de valider un dossier de financement.
  </p>
  <a href="{{ route('admin.financing-notification-templates.create') }}" class="btn-navy">
  ...
  <div class="col-md-6 col-xl-4">
    <div class="card-pro h-100 d-flex flex-column">
      ...
      <div style="padding:.875rem 1.25rem;border-top:1px solid var(--c-border);
        ...
        <a href="{{ route('admin.financing-notification-templates.edit',$t) }}"
           class="btn-navy btn-sm-pro" style="flex:1;justify-content:center;min-width:70px">
          <i class="fas fa-pen"></i> Modifier
        </a>
        <form action="{{ route('admin.financing-notification-templates.destroy',$t) }}" method="POST"
