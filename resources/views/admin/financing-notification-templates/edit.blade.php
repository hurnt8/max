...
@section('title','Modifier le modèle de notification — Financement')
@section('page_title','Modifier le modèle de notification — Financement')
...
<div class="row g-4">
  ...
  {{-- ── Colonne principale ── --}}
  <div class="col-xl-8">
    <form action="{{ route('admin.financing-notification-templates.update', $template) }}" method="POST" id="tplForm">
    @csrf @method('PUT')
        ...
        <div class="row g-3">
          ...
          <div class="col-sm-6">
            ...
            <select name="locale" class="form-control-pro" required>
              @foreach(['fr'=>'Français','en'=>'English','pl'=>'Polski','es'=>'Español','bg'=>'Български','hu'=>'Magyar','it'=>'Italiano','de'=>'Deutsch','lt'=>'Lietuvių','ro'=>'Română','lv'=>'Latviešu','nl'=>'Nederlands','pt'=>'Português','sk'=>'Slovenčina','el'=>'Ελληνικά'] as $code=>$label)
    ...
    @if(in_array($template->type, [\App\Models\FinancingNotificationTemplate::TYPE_VALIDATION, \App\Models\FinancingNotificationTemplate::TYPE_CONDITIONS, \App\Models\FinancingNotificationTemplate::TYPE_INSURANCE]))
    @php
      $isValidationType = $template->type === \App\Models\FinancingNotificationTemplate::TYPE_VALIDATION;
      $isInsuranceType  = $template->type === \App\Models\FinancingNotificationTemplate::TYPE_INSURANCE;
      $docxHeaderHelp = match(true) {
          ...
          default           => 'Document de référence, téléchargeable pour consultation ou conversion manuelle. L\'envoi automatique joint au contrat utilise le champ « Contenu » ci-dessus — pas ce DOCX.',
      };
      $docxEmptyHelp = match(true) {
          $isValidationType => 'Uploadez un fichier <code>.docx</code> contenant des placeholders <code>{variable}</code> — indispensable pour que « Valider » soit disponible sur les dossiers de financement de cette langue.',
    ...
    <div class="card-pro" id="docx-section">
      <div class="card-pro-hdr">
        ...
        <div style="display:flex;gap:.5rem;margin-left:auto;align-items:center;flex-wrap:wrap">
          <a href="{{ route('admin.financing-notification-templates.docx.download', $template) }}"
      ...
      <div class="card-pro-body">
        ...
        <form action="{{ route('admin.financing-notification-templates.docx.upload', $template) }}"
