...
<title>Contrat {{ $financing->reference }} · AURELIS CAPITAL GROUP Invest</title>
...
.pv-empty-ico i { font-size: 1.75rem; color: #DC2626; opacity: .4; }
...
<div class="pv">
  ...
  <aside class="pv-side" id="side">
    ...
    <a href="{{ route($panelPrefix.'.financings.show', $financing) }}" class="s-brand">
      <div class="s-logo">AC</div>
    ...
    <div class="s-doc">
      <div class="s-doc-badge"><i class="fas fa-file-contract"></i>&nbsp;Contrat de financement</div>
      <div class="s-doc-title">Aperçu du document</div>
      <div class="s-doc-sub">PDF · Lecture seule</div>
    </div>

    <div class="s-identity">
      <div class="s-ref">{{ $financing->reference }}</div>
      <div class="s-client-name">{{ $financing->name }}</div>
      @if($financing->email)
      <div class="s-client-email">{{ $financing->email }}</div>
      @endif
    </div>
    ...
    <div class="s-actions">
      <a href="{{ route($panelPrefix.'.financings.contract.pdf', $financing) }}"
         download="Contrat_{{ $financing->reference }}.pdf"
    ...
    <div class="s-nav">
      <a href="{{ route($panelPrefix.'.financings.show', $financing) }}" class="s-nav-item">
    ...
    <div class="s-back-wrap">
      <a href="{{ route($panelPrefix.'.financings.show', $financing) }}" class="s-back">
  ...
  <main class="pv-main">
    ...
    <div class="pv-mob-bar">
      ...
      <span class="pv-mob-ref">{{ $financing->reference }}</span>
      <span class="pv-mob-name">{{ $financing->name }}</span>
    </div>

    <div class="pv-bar">
      <nav class="pv-crumb" aria-label="Fil d'Ariane">
        <a href="{{ route($panelPrefix.'.financings.index') }}">Financements</a>
        <span class="pv-crumb-sep">/</span>
        <a href="{{ route($panelPrefix.'.financings.show', $financing) }}">{{ $financing->reference }}</a>
        <span class="pv-crumb-sep">/</span>
        <span class="pv-crumb-cur">Contrat de financement</span>
      </nav>
    ...
    <div class="pv-frame-wrap">
      <iframe
        class="pv-frame"
        src="{{ route($panelPrefix.'.financings.contract.pdf', $financing) }}#toolbar=1&navpanes=0&scrollbar=1&view=FitH"
        title="Contrat de financement — {{ $financing->reference }}"
        loading="eager"
      ></iframe>
    </div>
    @else
    <div class="pv-empty">
      ...
      <p>Uploadez le contrat signé depuis la fiche dossier.</p>
      <a href="{{ route($panelPrefix.'.financings.show', $financing) }}" class="pv-empty-cta">
        <i class="fas fa-upload"></i> Gérer le dossier
      </a>
    </div>
    @endif
...
document.addEventListener('keydown', function(e) {
  if (e.key === 'Escape') window.location.href = '{{ route($panelPrefix.'.financings.show', $financing) }}';
});
</script>
</body>
</html>
