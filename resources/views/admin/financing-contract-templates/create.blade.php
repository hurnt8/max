...
@section('title','Nouveau modèle de contrat — Financement')
@section('page_title','Nouveau modèle de contrat — Financement')

@section('content')

<div class="d-flex align-items-start justify-content-between flex-wrap gap-3 page-hdr">
  <div>
    <h4>Nouveau modèle de contrat — Financement</h4>
    <p>Renseignez le nom et la langue, vous uploaderez le fichier .docx à l'étape suivante.</p>
  </div>
  <a href="{{ route('admin.financing-contract-templates.index') }}" class="btn-ghost btn-sm-pro">
...
<form action="{{ route('admin.financing-contract-templates.store') }}" method="POST">
@csrf
    ...
    <div class="row g-3">
      <div class="col-sm-8">
        ...
        <input type="text" name="name" class="form-control-pro"
               value="{{ old('name') }}" placeholder="Ex: Contrat Financement Standard — FR" required>
      </div>
      <div class="col-sm-4">
        <label class="form-label-pro">Langue</label>
        <select name="locale" class="form-control-pro">
          <option value="">Automatique</option>
          @foreach(['fr'=>'Français','en'=>'English','pl'=>'Polski','es'=>'Español','bg'=>'Български','hu'=>'Magyar','it'=>'Italiano','de'=>'Deutsch','lt'=>'Lietuvių','ro'=>'Română','lv'=>'Latviešu','nl'=>'Nederlands','pt'=>'Português','sk'=>'Slovenčina','el'=>'Ελληνικά'] as $code=>$lbl)
...
<div style="display:flex;align-items:flex-start;gap:.875rem;padding:1rem 1.125rem;
  ...
  <div>
    ...
    <div style="font-size:.75rem;color:#1e40af;line-height:1.6">
      ...
      présentes dans le document et les remplacera avec les données du dossier de financement.
    </div>
  </div>
</div>

<div style="display:flex;gap:.75rem;justify-content:flex-end">
  <a href="{{ route('admin.financing-contract-templates.index') }}" class="btn-ghost">Annuler</a>
