...
@section('title','Modèles de contrats — Financement')
@section('page_title','Modèles de contrats — Financement')

@section('content')

<div class="d-flex align-items-start justify-content-between flex-wrap gap-3 page-hdr">
  <div>
    <h4>Modèles de contrats — Financement</h4>
    <p>Templates DOCX Word utilisés pour la génération des contrats de financement — indépendants des modèles « Prêt ».</p>
  </div>
  <a href="{{ route('admin.financing-contract-templates.create') }}" class="btn-navy">
...
<div class="card-pro" style="text-align:center;padding:4rem 2rem">
  ...
  <p style="color:var(--c-muted);font-size:.8375rem;margin-bottom:1.25rem">
    Créez votre premier template pour personnaliser les contrats de financement générés.
  </p>
  <a href="{{ route('admin.financing-contract-templates.create') }}" class="btn-navy">
  ...
  <div class="col-md-6 col-xl-4">
    <div class="card-pro h-100 d-flex flex-column"
      ...
      <div style="padding:.875rem 1.25rem;border-top:1px solid var(--c-border);
        ...
        @if($hasDocx)
        <a href="{{ route('admin.financing-contract-templates.docx.download', $t) }}"
           class="btn-ghost btn-sm-pro" title="Télécharger le template DOCX">
          <i class="fas fa-download"></i>
        </a>
        <a href="{{ route('admin.financing-contract-templates.docx.preview', $t) }}"
           class="btn-ghost btn-sm-pro" target="_blank" title="Aperçu DOCX avec données demo">
          <i class="fas fa-file-word"></i>
        </a>
        @endif
        <a href="{{ route('admin.financing-contract-templates.edit',$t) }}"
           class="btn-navy btn-sm-pro" style="flex:1;justify-content:center;min-width:70px">
          <i class="fas fa-pen"></i> Modifier
        </a>
        @if(!$t->is_default)
        <form action="{{ route('admin.financing-contract-templates.destroy',$t) }}" method="POST"
