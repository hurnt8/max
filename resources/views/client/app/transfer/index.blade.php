...
@php $currency = $user->currency ?? config('AURELIS CAPITAL GROUP.default_currency'); @endphp
...
<div style="margin:.875rem 1.25rem 0;padding:.875rem 1rem;background:rgba(248,113,113,.08);border:1px solid rgba(248,113,113,.25);border-left:3px solid #f87171;border-radius:0 14px 14px 0;display:flex;align-items:flex-start;gap:.625rem">
  ...
  <div>
    ...
    <div style="font-size:.73rem;color:var(--ca-text-3);line-height:1.5">Votre solde est négatif ({{ number_format((float)$user->balance,2,',',' ') }} {{ $user->currency ?? config('AURELIS CAPITAL GROUP.default_currency') }}). Les virements sont désactivés jusqu'à la régularisation de votre compte.</div>
  </div>
</div>
@endif
