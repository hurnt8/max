...
@section('title','Toutes les demandes — Super Admin')
...
/* ── Super-admin badge ── */
.sa-badge{display:inline-flex;align-items:center;gap:.3rem;font-size:.62rem;font-weight:800;padding:.15rem .5rem;border-radius:6px;background:rgba(124,58,237,.1);color:#7c3aed;border:1px solid rgba(124,58,237,.2);vertical-align:middle}
...
@media(max-width:640px){
  ...
  table.li-tbl tbody tr{display:block;background:#fff;border:1px solid var(--c-border);border-radius:10px;margin-bottom:.75rem;overflow:hidden}
  table.li-tbl tbody tr:hover td{background:transparent}
  table.li-tbl tbody td{display:flex;justify-content:space-between;align-items:center;padding:.4rem 1rem;border-bottom:1px solid #f3f4f6;font-size:.8rem;min-height:36px;border-left:none!important}
  table.li-tbl tbody td:last-child{border-bottom:none;justify-content:center;gap:.625rem;flex-wrap:wrap;padding:.625rem 1rem;background:#fafbfc;border-top:1px solid var(--c-border)}
  table.li-tbl tbody td:last-child::before{display:none}
  table.li-tbl tbody td:first-child{padding:.75rem 1rem;background:#fafbfc;border-bottom:1px solid var(--c-border);min-height:44px}
  table.li-tbl tbody td:first-child::before{display:none}
  table.li-tbl tbody td[data-label]:not([data-label=""])::before{content:attr(data-label);font-size:.65rem;font-weight:700;color:var(--c-muted);text-transform:uppercase;letter-spacing:.05em;flex-shrink:0;margin-right:.75rem;white-space:nowrap}
...
<div class="li-header">
  <div>
    ...
    <p class="li-header-sub">
      Supervision globale · {{ $stats['total'] }} dossier(s)
  ...
  <a href="{{ route('super-admin.loans.create') }}" class="btn-navy">
...
<div class="li-kpi-row">
  <div class="li-kpi li-kpi--navy">
    ...
    <div><div class="li-kpi-val">{{ $stats['total'] }}</div><div class="li-kpi-lbl">Total</div></div>
  </div>
  <div class="li-kpi li-kpi--amber">
    <div class="li-kpi-icon"><i class="fas fa-hourglass-half"></i></div>
    <div><div class="li-kpi-val">{{ $stats['pending'] }}</div><div class="li-kpi-lbl">En attente</div></div>
  </div>
  <div class="li-kpi li-kpi--blue">
    <div class="li-kpi-icon"><i class="fas fa-paper-plane"></i></div>
    <div><div class="li-kpi-val">{{ $stats['contract_sent'] + $stats['contract_signed'] }}</div><div class="li-kpi-lbl">Contrats en cours</div></div>
  </div>
  <div class="li-kpi li-kpi--green">
    <div class="li-kpi-icon"><i class="fas fa-flag-checkered"></i></div>
    <div><div class="li-kpi-val">{{ $stats['finalized'] }}</div><div class="li-kpi-lbl">Finalisées</div></div>
  </div>
</div>
...
<div class="li-filters-strip">
  ...
  @if($count > 0 || $val === '')
  <a href="{{ route('super-admin.loans.index', array_merge(request()->except('status','page'), $val ? ['status'=>$val] : [])) }}"
...
{{-- ── BARRE DE RECHERCHE ── --}}
<form method="GET" action="{{ route('super-admin.loans.index') }}" class="li-search-bar">
  @if($currentStatus)
      ...
      <tbody>
        ...
        <tr>
          ...
          <td data-label="Référence">
            <a href="{{ route('super-admin.loans.show', $loan) }}" class="li-ref-link">
              {{ $loan->reference }}
            </a>
          ...
          <td data-label="Admin">
            ...
            @else
              <span class="li-sub">— Non assigné</span>
            @endif
          </td>
          ...
          <td data-label="">
            <div class="li-act">
              ...
              {{-- Voir le dossier --}}
              <a href="{{ route('super-admin.loans.show', $loan) }}"
              ...
              {{-- Contrat / statut contextuel --}}
              @if(in_array($loan->status, ['draft','pending','validated']))
              <a href="{{ route('super-admin.loans.contract', $loan) }}"
              ...
              <span class="btn-icon" style="opacity:.3;cursor:default" title="Finalisé">
                <i class="fas fa-flag-checkered"></i>
              </span>
              @elseif($loan->status === 'rejected')
              <span class="btn-icon" style="opacity:.3;cursor:default;color:#dc2626" title="Rejeté">
                <i class="fas fa-ban"></i>
              </span>
              @endif

              {{-- Viewer contrat (si PDF disponible) --}}
              @if($loan->contract_pdf_path)
              <a href="{{ route('super-admin.loans.contract.viewer', $loan) }}"
                 class="btn-icon" title="Visualiser le contrat" style="color:#dc2626">
                <i class="fas fa-expand-alt"></i>
              </a>
              @endif

              {{-- Viewer assurance (si attestation disponible) --}}
              @if($loan->insurance_pdf_path)
              <a href="{{ route('super-admin.loans.insurance.viewer', $loan) }}"
                 class="btn-icon" title="Visualiser l'attestation" style="color:#16a34a">
