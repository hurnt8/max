@php
$texts = [
    'fr' => [
        'title'   => 'Contrat N°'.$financing->reference.' reçu',
        'sub'     => 'Accusé de réception',
        'greeting'=> 'Madame / Monsieur '.$financing->name.',',
        'intro'   => 'Nous accusons bonne réception de votre contrat de financement signé (Référence : <strong>'.$financing->reference.'</strong>).',
        'next'    => 'Notre équipe de gestion va procéder au traitement final de votre dossier. Les coordonnées du compte et les modalités de versement vous seront communiquées sous <strong>24 à 48 heures</strong>.',
        'closing' => 'Cordialement,',
        'team'    => "L'équipe AURELIS CAPITAL GROUP",
    ],
    'en' => [
        'title'   => 'Contract N°'.$financing->reference.' received',
        'sub'     => 'Acknowledgement of receipt',
        'greeting'=> 'Dear '.$financing->name.',',
        'intro'   => 'We confirm receipt of your signed financing contract (Reference: <strong>'.$financing->reference.'</strong>).',
        'next'    => 'Our management team will proceed with the final processing of your file. Payment account details will be communicated within <strong>24 to 48 hours</strong>.',
        'closing' => 'Yours sincerely,',
        'team'    => 'The AURELIS CAPITAL GROUP team',
    ],
    'es' => [
        'title'   => 'Contrato N°'.$financing->reference.' recibido',
        'sub'     => 'Acuse de recibo',
        'greeting'=> 'Estimado/a '.$financing->name.',',
        'intro'   => 'Confirmamos la recepción de su contrato de financiación firmado (Referencia: <strong>'.$financing->reference.'</strong>).',
        'next'    => 'Nuestro equipo de gestión procederá al tratamiento final de su expediente. Los datos de la cuenta de pago le serán comunicados en <strong>24 a 48 horas</strong>.',
        'closing' => 'Atentamente,',
        'team'    => 'El equipo AURELIS CAPITAL GROUP',
    ],
    'pl' => [
        'title'   => 'Umowa nr '.$financing->reference.' odebrana',
        'sub'     => 'Potwierdzenie odbioru',
        'greeting'=> 'Szanowny/a '.$financing->name.',',
        'intro'   => 'Potwierdzamy otrzymanie Państwa podpisanej umowy finansowania (Nr referencyjny: <strong>'.$financing->reference.'</strong>).',
        'next'    => 'Nasz zespół przystąpi do ostatecznego rozpatrzenia Państwa wniosku. Dane rachunku bankowego zostaną przekazane w ciągu <strong>24 do 48 godzin</strong>.',
        'closing' => 'Z poważaniem,',
        'team'    => 'Zespół AURELIS CAPITAL GROUP',
    ],
    'bg' => [
        'title'   => 'Договор №'.$financing->reference.' получен',
        'sub'     => 'Потвърждение за получаване',
        'greeting'=> 'Уважаеми/а '.$financing->name.',',
        'intro'   => 'Потвърждаваме получаването на вашия подписан договор за финансиране (Референция: <strong>'.$financing->reference.'</strong>).',
        'next'    => 'Нашият екип за управление ще пристъпи към окончателната обработка на вашето досие. Данните на сметката ще ви бъдат съобщени в рамките на <strong>24 до 48 часа</strong>.',
        'closing' => 'С уважение,',
        'team'    => 'Екипът на AURELIS CAPITAL GROUP',
    ],
    'hu' => [
        'title'   => 'A(z) '.$financing->reference.' sz. szerződés beérkezett',
        'sub'     => 'Átvétel visszaigazolása',
        'greeting'=> 'Tisztelt '.$financing->name.'!',
        'intro'   => 'Visszaigazoljuk aláírt finanszírozási szerződésének beérkezését (Hivatkozási szám: <strong>'.$financing->reference.'</strong>).',
        'next'    => 'Ügykezelő csapatunk elvégzi ügye végleges feldolgozását. A fizetési számla adatait <strong>24–48 órán belül</strong> közöljük Önnel.',
        'closing' => 'Tisztelettel,',
        'team'    => 'A AURELIS CAPITAL GROUP csapata',
    ],
    'it' => [
        'title'   => 'Contratto N°'.$financing->reference.' ricevuto',
        'sub'     => 'Conferma di ricezione',
        'greeting'=> 'Gentile '.$financing->name.',',
        'intro'   => 'Confermiamo la ricezione del tuo contratto di finanziamento firmato (Riferimento: <strong>'.$financing->reference.'</strong>).',
        'next'    => 'Il nostro team di gestione procederà alla lavorazione finale della tua pratica. I dati del conto ti saranno comunicati entro <strong>24-48 ore</strong>.',
        'closing' => 'Cordiali saluti,',
        'team'    => 'Il team AURELIS CAPITAL GROUP',
    ],
    'de' => [
        'title'   => 'Vertrag Nr. '.$financing->reference.' eingegangen',
        'sub'     => 'Empfangsbestätigung',
        'greeting'=> 'Sehr geehrte Damen und Herren '.$financing->name.',',
        'intro'   => 'Wir bestätigen den Eingang Ihres unterschriebenen Finanzierungsvertrags (Referenz: <strong>'.$financing->reference.'</strong>).',
        'next'    => 'Unser Verwaltungsteam wird nun die endgültige Bearbeitung Ihrer Akte vornehmen. Die Kontodaten werden Ihnen innerhalb von <strong>24 bis 48 Stunden</strong> mitgeteilt.',
        'closing' => 'Mit freundlichen Grüßen,',
        'team'    => 'Das AURELIS CAPITAL GROUP Team',
    ],
    'lt' => [
        'title'   => 'Sutartis Nr. '.$financing->reference.' gauta',
        'sub'     => 'Gavimo patvirtinimas',
        'greeting'=> 'Gerbiamas (-a) '.$financing->name.',',
        'intro'   => 'Patvirtiname jūsų pasirašytos finansavimo sutarties gavimą (Nuoroda: <strong>'.$financing->reference.'</strong>).',
        'next'    => 'Mūsų valdymo komanda atliks galutinį jūsų bylos apdorojimą. Sąskaitos duomenys jums bus pateikti per <strong>24–48 valandas</strong>.',
        'closing' => 'Pagarbiai,',
        'team'    => 'AURELIS CAPITAL GROUP komanda',
    ],
    'ro' => [
        'title'   => 'Contractul nr. '.$financing->reference.' primit',
        'sub'     => 'Confirmare de primire',
        'greeting'=> 'Stimate/Stimată '.$financing->name.',',
        'intro'   => 'Confirmăm primirea contractului dumneavoastră de finanțare semnat (Referință: <strong>'.$financing->reference.'</strong>).',
        'next'    => 'Echipa noastră de gestiune va proceda la procesarea finală a dosarului dumneavoastră. Datele contului vă vor fi comunicate în <strong>24 până la 48 de ore</strong>.',
        'closing' => 'Cu stimă,',
        'team'    => 'Echipa AURELIS CAPITAL GROUP',
    ],
    'lv' => [
        'title'   => 'Līgums Nr. '.$financing->reference.' saņemts',
        'sub'     => 'Saņemšanas apstiprinājums',
        'greeting'=> 'Godātais/Godātā '.$financing->name.',',
        'intro'   => 'Apstiprinām jūsu parakstītā finansējuma līguma saņemšanu (Atsauce: <strong>'.$financing->reference.'</strong>).',
        'next'    => 'Mūsu pārvaldības komanda veiks jūsu lietas galīgo apstrādi. Konta dati jums tiks paziņoti <strong>24 līdz 48 stundu</strong> laikā.',
        'closing' => 'Ar cieņu,',
        'team'    => 'AURELIS CAPITAL GROUP komanda',
    ],
    'nl' => [
        'title'   => 'Contract nr. '.$financing->reference.' ontvangen',
        'sub'     => 'Ontvangstbevestiging',
        'greeting'=> 'Geachte heer/mevrouw '.$financing->name.',',
        'intro'   => 'Wij bevestigen de ontvangst van uw ondertekende financieringsovereenkomst (Referentie: <strong>'.$financing->reference.'</strong>).',
        'next'    => 'Ons beheerteam zal overgaan tot de definitieve verwerking van uw dossier. De gegevens van de betaalrekening worden u binnen <strong>24 tot 48 uur</strong> meegedeeld.',
        'closing' => 'Met vriendelijke groet,',
        'team'    => 'Het team van AURELIS CAPITAL GROUP',
    ],
    'pt' => [
        'title'   => 'Contrato N.º'.$financing->reference.' recebido',
        'sub'     => 'Confirmação de receção',
        'greeting'=> 'Caro(a) '.$financing->name.',',
        'intro'   => 'Confirmamos a receção do seu contrato de financiamento assinado (Referência: <strong>'.$financing->reference.'</strong>).',
        'next'    => 'A nossa equipa de gestão irá proceder ao tratamento final do seu processo. Os dados da conta de pagamento ser-lhe-ão comunicados dentro de <strong>24 a 48 horas</strong>.',
        'closing' => 'Atenciosamente,',
        'team'    => 'A equipa AURELIS CAPITAL GROUP',
    ],
];
$t = $texts[$locale] ?? $texts['fr'];
@endphp

<x-email-layout
    :title="$t['title']"
    :subtitle="$t['sub']"
    accent="teal"
>

  <p class="greeting">{{ $t['greeting'] }}</p>

  <p class="body-text">{!! $t['intro'] !!}</p>

  <div class="alert alert-info">
    <p>{!! $t['next'] !!}</p>
  </div>

  <p class="closing">
    {{ $t['closing'] }}<br>
    <strong>{{ $t['team'] }}</strong>
  </p>

</x-email-layout>
