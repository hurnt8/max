<?php

return [
    'title' => 'Kundizzjonijiet u termini',
    'sections' => [
        [
            'title' => '1. Introduzzjoni',
            'content' => 'Merħba fis-' . site_name() . '. Billi tuża s-sit u s-servizzi tagħna, taqbel li tikkonforma u tkun marbut bil-kundizzjonijiet u t-termini li ġejjin. Jekk jogħġbok aqra dawn il-kundizzjonijiet b’attenzjoni qabel ma tuża s-sit tagħna jew tieħu self magħna.'
        ],
        [
            'title' => '2. Użu tas-sit',
            'content' => 'Billi taċċessa s-sit tagħna, taqbel li tuża s-servizzi tagħna biss għal skopijiet legali u skont dawn il-kundizzjonijiet. Taqbel li ma tużax is-sit tagħna b’xi mod li jikkawża ħsara, interruzzjoni jew interferenza mas-servizzi tagħna jew bl-aċċess ta’ utenti oħra.'
        ],
        [
            'title' => '3. Servizzi tas-self',
            'content' => 'Is-' . site_name() . ' toffri diversi tipi ta’ self biex tilħaq il-bżonnijiet finanzjarji tiegħek. Is-self kollha huma soġġetti għall-approvazzjoni ibbażata fuq l-evalwazzjoni tagħna tal-kapaċità tiegħek ta’ ħlas u kredibilità. It-termini speċifiċi ta’ kull self, inklużi r-rati ta’ imgħax, il-maturitajiet u t-termini ta’ ħlas, se jkunu dettaljati fl-aqbil tiegħek.'
        ],
        [
            'title' => '4. Spejjeż tal-kuntratt u tal-assigurazzjoni',
            'content' => 'Meta tieħu self ma’ ' . site_name() . ', jistgħu jiġu applikati spejjeż tal-kuntratt u tal-assigurazzjoni. Dan il-miżien jkopri l-ispejjeż amministrattivi assoċjati mal-impostazzjoni u l-amministrazzjoni tas-self tiegħek. Id-dettalji speċifiċi ta’ dawn l-ispejjeż se jkunu ċari fl-aqbil tiegħek. Int mitlub li tħallas dawn il-miżati skond it-termini speċifikati.'
        ],
        [
            'title' => '5. Kunfidenzjalità',
            'content' => 'Aħna nieħdu l-privatezza tal-informazzjoni personali tiegħek b’mod serju ħafna. Jekk jogħġbok ara l-<a href="#">politika ta’ privatezza</a> tagħna għal aktar dettalji dwar kif niġbru, nużaw u nipproteġu l-informazzjoni personali tiegħek.'
        ],
        [
            'title' => '6. Tmiem',
            'content' => 'Aħna nżommu d-dritt li ntemmu jew suspendu l-aċċess tiegħek għal s-sit tagħna u s-servizzi tagħna mingħajr avviż jekk nemmnu li kkieħed uċuħ dawn il-kundizzjonijiet jew jekk nikkunsidraw li huwa meħtieġ biex nipproteġu l-interessi tagħna jew dawk ta’ utenti oħra.'
        ],
        [
            'title' => '7. Limitazzjoni ta’ responsabbiltà',
            'content' => 'Is-' . site_name() . ' ma tkunx responsabbli għal xi telf indirett, speċjali jew konsekuttiv jew ħsara li tirriżulta mill-użu jew l-inabbilità li tuża s-sit tagħna jew is-servizzi tagħna, inkluż it-telf ta’ profitti, it-telf ta’ data jew interruzzjoni tan-negozju, anki jekk ġew avżati dwar il-possibbiltà ta’ din il-ħsara.'
        ],
        [
            'title' => '8. Bidliet fil-kundizzjonijiet',
            'content' => 'Aħna nistgħu nbiddlu dawn il-kundizzjonijiet u t-termini minn żmien għal żmien biex jirriflettu bidliet fil-prattiċi tagħna jew fil-liġijiet applikabbli. Aħna ninfurmawk dwar xi bidliet sostanzjali billi nqiegħdu t-termini ġodda fis-sit tagħna. L-użu kontinwu tiegħek tas-sit tagħna wara dawn il-bidliet jikkostitwixxi l-aċċettazzjoni tiegħek tat-termini ġodda.'
        ],
        [
            'title' => '9. Kontatt',
            'content' => 'Jekk għandek xi mistoqsijiet jew tħassib rigward dawn il-kundizzjonijiet u t-termini, jekk jogħġbok ikkuntattjana fuq:<br><br>' . site_name() . '<br>Indirizz: Calcea Victoriei 118 Settore 1 010093 Bukarest<br>Email: ' . site_email() . '<br>Telefon: ' . site_phone() . ' '
        ],
    ],

];
